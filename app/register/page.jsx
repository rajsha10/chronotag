...
// ChronoTag ABI - this should be the ABI of your deployed contract
...
export default function RegisterPage() {
  ...
  const [chronoTag, setChronoTag] = useState("")
  ...
  const registerIPOnBlockchain = async (ipfsHash) => {
    try {
      ...
      // Get ChronoTag from event
      const event = receipt.events.find((event) => event.event === "IPRegistered")
      if (event) {
        const chronoTagValue = event.args.chronoTag.toString()
        return chronoTagValue
      }

      throw new Error("Failed to get ChronoTag from transaction")
    } catch (error) {
