...
export default function SearchPage() {
      ...
      try {
        ...
        if (!mmsdkRef.current) {
          ...
          mmsdkRef.current = new MetaMaskSDK({
            dappMetadata: {
              name: "ChronoTag Research Platform",
  ...
  const initializeContract = async () => {
    try {
      ...
      const chronoTagContract = new ethers.Contract(contractAddress, contractABI, signer)
      console.log("Contract instance created")
      setContract(chronoTagContract)

      // Get access fee from contract
      try {
        const fee = await chronoTagContract.accessFee()
  ...
  const handleSearch = async (e) => {
    ...
    const chronoTagPattern = /^\d{1,12}$/
    if (!chronoTagPattern.test(searchQuery)) {
      setError("Invalid Chrono Tag format. Please enter a number up to 12 digits")
      return
    }

    if (!isConnected) {
      setError("Please connect your wallet first to search for Chrono Tags")
      return
    }
    ...
    try {
      // Call the contract's searchResearchByChronoTag function
      const result = await contract.searchResearchByChronoTag(searchQuery)
      ...
      console.error("Error searching for Chrono Tag:", err)
      setError(
        "Error retrieving data from blockchain. The Chrono Tag may not exist or there might be a connection issue",
      )
    } finally {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // IMPORTANT: accessIP expects the chronoTag as a number, not a string
          ...
          <CardHover className="p-8 mb-8">
            <form onSubmit={handleSearch} className="space-y-6">
              <div className="relative">
                <Input
                  type="text"
                  placeholder="Enter 12-digit Chrono Tag (e.g., 123456789012)"
                  value={searchQuery}
                ...
                <div className="space-y-6">
                  ...
                  {!ipfsHash && (
                    <div className="bg-secondary/50 rounded-lg p-4 flex items-start">
                      ...
                      <p className="text-sm">
                        This record was verified on the blockchain with Chrono Tag #{searchResult.id}. To access the
                  ...
                  {showPaymentSection && !ipfsHash && (
                    ...
                    >
                      ...
                      <p className="text-muted-foreground mb-4">
                        Pay {accessFee} EDU to access the full research document.
                      </p>
